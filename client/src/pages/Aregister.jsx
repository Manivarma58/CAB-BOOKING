...
const Aregister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await adminAPI.register({ name, email, password });
      ...
      <div className="flex flex-1 items-center justify-center px-4 py-12 sm:px-6 lg:px-8 bg-slate-950/40">
        <div className="w-full max-w-md space-y-8 rounded-3xl border border-slate-800 bg-slate-950 p-8 shadow-2xl">
          <div className="text-center space-y-2">
            ...
            <h2 className="text-3xl font-extrabold tracking-tight">Create Admin Profile</h2>
            <p className="text-sm text-slate-400">
              Initialize a new administrative manager or{' '}
              <Link to="/admin/login" className="font-semibold text-yellow-500 hover:text-yellow-450">
                Log In
              </Link>
            </p>
          </div>
